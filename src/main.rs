...
fn action_button(ui: &mut egui::Ui, label: &str, enabled: bool, primary: bool) -> egui::Response {
    ...
    ui.add_enabled(enabled, egui::Button::new(RichText::new(label).size(14.0).color(if enabled { TEXT } else { DISABLED }))
        .min_size(Vec2::new(0.0, 40.0)).rounding(Rounding::same(6.0)).fill(fill).stroke(Stroke::new(1.0, stroke)))
}
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if !self.hotkeys_started {
            let skills = Arc::clone(&self.skills); let running = Arc::clone(&self.running);
            let generation = Arc::clone(&self.generation); let status = Arc::clone(&self.status);
            thread::spawn(move || {
                let mut previous_p = false; let mut previous_s = false;
                loop {
                    unsafe {
        ...
        egui::CentralPanel::default().frame(egui::Frame::none().fill(BG).inner_margin(egui::Margin::same(20.0))).show(ctx, |ui| {
            ui.add_space(2.0);
            ...
            ui.add_space(14.0);
            egui::Frame::none().fill(PANEL).stroke(Stroke::new(1.0, BORDER)).rounding(Rounding::same(8.0)).inner_margin(egui::Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
                let columns = [("En.", 55.0), ("Slot", 50.0), ("Key", 130.0), ("Interval (ms)", 145.0), ("Press (ms)", 125.0), ("Release delay (ms)", 155.0)];
                ui.horizontal(|ui| { for (label, width) in columns { ui.allocate_ui_with_layout(Vec2::new(width, 20.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.label(RichText::new(label).size(11.0).color(DIM)); }); } });
                ui.add_space(4.0);
                if let Ok(mut skills) = self.skills.write() {
                    for (index, skill) in skills.iter_mut().enumerate() {
                        ...
                        egui::Frame::none().fill(row_color).inner_margin(egui::Margin::symmetric(4.0, 6.0)).show(ui, |ui| {
                            ui.set_min_height(52.0);
                            ui.horizontal_centered(|ui| {
                                ui.allocate_ui_with_layout(Vec2::new(55.0, 52.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { toggle(ui, &mut skill.enabled); });
                                ui.allocate_ui_with_layout(Vec2::new(50.0, 52.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.label(RichText::new((index + 1).to_string()).size(14.0).color(TEXT)); });
                                ui.allocate_ui_with_layout(Vec2::new(130.0, 52.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.add_sized([114.0, 34.0], egui::TextEdit::singleline(&mut skill.key).horizontal_align(egui::Align::Center)); });
                                ui.allocate_ui_with_layout(Vec2::new(145.0, 52.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.add_sized([129.0, 34.0], egui::DragValue::new(&mut skill.interval_ms).range(MIN_INTERVAL..=3_600_000).speed(1.0)); });
                                ui.allocate_ui_with_layout(Vec2::new(125.0, 52.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.add_sized([109.0, 34.0], egui::DragValue::new(&mut skill.press_ms).range(MIN_PRESS..=60_000).speed(1.0)); });
                                ui.allocate_ui_with_layout(Vec2::new(155.0, 52.0), egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.add_sized([139.0, 34.0], egui::DragValue::new(&mut skill.release_ms).range(MIN_RELEASE..=60_000).speed(1.0)); });
                            });
                        });
                    }
                }
